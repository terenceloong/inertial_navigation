...
Gamma(8,3) = 0.5*dt^2;
Gamma(9,3) = dt;
P = diag([50,30,5, 50,30,5, 50,30,5])^2;
Q = diag([5, 5, 5])^2;
...
h6 = plot3(X(1),X(4),X(7), 'Color','c', 'Marker','.', 'MarkerSize',16); %estimation pint
h7 = plot3(X(1),X(4),X(7), 'Color','c', 'LineWidth',1); %estimation trajectory
h8 = plot3([vehicle(1,2),X(1)],[vehicle(1,3),X(4)],[vehicle(1,4),X(7)], 'Color','g', 'LineStyle','--');
...
for k=2:n
    ...
    set(h6, 'XData',X(1), 'YData',X(4), 'ZData',X(7)); %estimation pint
    set(h7, 'XData',filter_X(1:k,1), 'YData',filter_X(1:k,4), 'ZData',filter_X(1:k,7)); %estimation trajectory
    set(h8, 'XData',[Rv(1),X(1)], 'YData',[Rv(2),X(4)], 'ZData',[Rv(3),X(7)]);
    pause(dt);
end
...
subplot(3,1,3)
plot(t, filter_X(:,7)-target(:,4), 'LineWidth',2)
grid on
xlabel('\itt\rm(s)')
...
subplot(3,1,2)
plot(t, filter_X(:,5)-target(:,6), 'LineWidth',2)
grid on
xlabel('\itt\rm(s)')
ylabel('\it\deltav_y\rm(m/s)')

subplot(3,1,3)
plot(t, filter_X(:,8)-target(:,7), 'LineWidth',2)
grid on
xlabel('\itt\rm(s)')
